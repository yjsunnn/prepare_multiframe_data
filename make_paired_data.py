...
def realesrgan_degradation(batch,  args_degradation, use_usm=True, sf=4, resize_lq=True):
    ...
    im_gts = batch['gt'].cuda()

    lqs = []
    gts = []

    for im_gt_index in range(len(im_gts)):
        im_gt = im_gts[im_gt_index]
        if use_usm:
        ...
        print("im_gt.shape: {}, im_lq.shape: {}".format(im_gt.shape, im_lq.shape))
    ...
    lq_final = torch.stack(lqs)
    gt_final = torch.stack(gts)
    # gt_final = gts[0].to(im_gts.device)
...
with torch.no_grad():
    for epoch in range(epochs):
        for num_batch, batch in enumerate(train_dataloader):
            lr_batchs, gt_batch = realesrgan_degradation(batch, args_degradation=args_degradation)
            sr_bicubic_batch = F.interpolate(lr_batchs[:, 0, :, :, :], size=(gt_batch.size(-2), gt_batch.size(-1)), mode='bicubic',)

            for i in range(batch_size):
                step += 1
                print('process {} images...'.format(step))
                for frame in range(lr_batchs.shape[1]):
                    lr = lr_batchs[i, frame, ...]
                    lr_save_path = os.path.join(lr_path, '{}_{}.png'.format(str(step).zfill(7), frame))
                    cv2.imwrite(lr_save_path, 255 * lr.detach().cpu().squeeze().permute(1, 2, 0).numpy()[..., ::-1])

                    # gt = gt_batch[i, frame, ...]
                    # gt_save_path = os.path.join(gt_path, '{}_{}.png'.format(str(step).zfill(7), frame))
            ...
            del lr_batchs, gt_batch, sr_bicubic_batch
